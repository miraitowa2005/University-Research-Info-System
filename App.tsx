      ...
      <main className="flex-1 overflow-y-auto relative">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 pb-12">
          ...
          {activeView === 'stats' && (
            <StatsContainer 
              data={(function(){
                const base = applyFilters(researchData);
                const deptId = (currentUser as any)?.dept_id;
                if (deptId && (users || []).length) {
                  const ids = (users || []).filter(u => (u as any)?.dept_id === deptId).map(u => String(u.id));
                  return base.filter(it => ids.includes(String((it as any)?.authorId)));
                }
                return base;
              })()} 
              currentUser={currentUser}
...
const ResearchAdminDashboard = ({ data, onNavigate, onExport }: { data: ResearchItem[], onNavigate: (v: string) => void, onExport: (items: ResearchItem[]) => void }) => {
  ...
  const [deptFunding, setDeptFunding] = React.useState<number>(0);
  const [fundingLoading, setFundingLoading] = React.useState<boolean>(false);
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        setFundingLoading(true);
        const v = await researchAPI.fundingByDept(1);
        if (!cancelled) setDeptFunding(Number(v || 0));
      } catch {
      } finally {
  ...
  const totalFundingStr = Number(deptFunding || 0).toLocaleString('zh-CN');
  ...
  return (
     <div className="space-y-6">
       ...
       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <DashboardCard title="累计经费 (万元)" value={fundingLoading ? '...' : totalFundingStr} icon={<Briefcase className="text-blue-600" />} bg="bg-blue-50" />
