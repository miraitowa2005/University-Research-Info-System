    ...
    try {
      ...
      if (!deptCode && currentUser?.department) {
         try {
            ...
            console.warn("部门标准化失败，将仅提交名称", err);
         }
      }
      ...
      const payload = {
        ...
        email: currentUser!.email,
        department: currentUser!.department,
        department_code: deptCode, // 传递计算后的 code
        employee_id: (currentUser as any)?.employee_id,
        gender: (currentUser as any)?.gender,
        birth_date: (currentUser as any)?.birth_date,
        phone: (currentUser as any)?.phone,
      ...
      const me = await usersAPI.getMe();
      onCurrentUserChange(me);
      toast.success('个人档案已成功保存！');
    } catch (e: any) {
      console.error(e);
      toast.error(e.message || '保存失败，请检查网络或数据格式');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mt-4">
        ...
        {accountTab === 'academic' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs text-gray-500 mb-1">研究方向</label>
              <input
                className="w-full border rounded px-3 py-2 text-sm"
                value={(currentUser as any)?.research_direction || ''}
                onChange={(e) => onCurrentUserChange({ ...(currentUser as any), research_direction: e.target.value })}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mt-4">
        ...
        {/* ... 省略中间渲染部分，逻辑同上，记得给内部所有 button 加上 type="button" ... */}
        
        {accountTab === 'experience' && (
          <>
             {/* 表单部分 */}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                <select className="border rounded px-3 py-2 text-sm" value={expForm.type} onChange={(e) => onExpFormChange({ ...expForm, type: e.target.value })}>
                  <option value="education">教育经历</option>
                  <option value="work">工作经历</option>
                </select>
                ...
                <input className="border rounded px-3 py-2 text-sm" placeholder="标题/岗位" value={expForm.title} onChange={(e) => onExpFormChange({ ...expForm, title: e.target.value })} />
                <input className="border rounded px-3 py-2 text-sm" placeholder="单位/学校" value={expForm.institution} onChange={(e) => onExpFormChange({ ...expForm, institution: e.target.value })} />
                <input className="border rounded px-3 py-2 text-sm md:col-span-2" placeholder="描述" value={expForm.description} onChange={(e) => onExpFormChange({ ...expForm, description: e.target.value })} />
             </div>
             <div className="flex justify-end mb-4">
                <button 
                  type="button" 
                ...
                {(experiences || []).map((exp: any) => (
                  <div key={exp.id} className="relative group">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg hover:bg-white hover:shadow-md border border-transparent hover:border-gray-100 transition-all">
                      ...
                      <div className="text-sm text-indigo-600 font-medium mt-1">{exp.institution}</div>
                      <div className="mt-2 text-right space-x-3">
                        <button type="button" className="text-red-600 hover:text-red-900 text-xs" onClick={async () => {
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mt-4">
        ...
        {accountTab === 'security' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* 密码修改部分略，记得给修改密码按钮也加上 type="button" */}
             <div>
                <label className="block text-xs text-gray-500 mb-1">旧密码</label>
                <input type="password" className="w-full border rounded px-3 py-2 text-sm" value={oldPwd} onChange={(e) => setOldPwd(e.target.value)} />
             </div>
             ...
             <div className="flex items-end">
                <button 
                  ...
                  onClick={async () => {
                    if(!oldPwd || !newPwd) return toast.error('请填写密码');
                    try { await usersAPI.changeMyPassword(oldPwd, newPwd); toast.success('密码修改成功'); setOldPwd(''); setNewPwd(''); } catch(e:any) { toast.error(e.message || '失败'); }
                  }}
                >修改密码</button>
             </div>
          </div>
      ...
      {/* 底部保存按钮 (冗余，可以删除，或者加上 type="button") */}
